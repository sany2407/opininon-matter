...
import { getBlogById, getCategories, updateBlogAuth } from "@/lib/api";
...
export default function EditBlogPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [blog, cats] = await Promise.all([getBlogById(id), getCategories()]);
        setCategories(cats);
        if (blog) {
          setTitle(blog.title);
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="ml-64 flex-1 p-8">
        <div className="max-w-4xl">
          <h1 className="font-display-lg text-display-lg-mobile text-primary mb-2">Edit Blog</h1>
          <p className="font-body-md text-on-surface-variant mb-8">Update your article content.</p>
